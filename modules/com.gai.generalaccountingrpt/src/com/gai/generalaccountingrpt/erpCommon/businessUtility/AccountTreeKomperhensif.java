 ...
 */
public class AccountTreeKomperhensif {
  private static Logger log4j = Logger.getLogger(AccountTreeKomperhensif.class);
  private VariablesSecureApp vars;
  private ConnectionProvider conn;
  private AccountTreeKomperhensifData[] accountsFacts;
  private AccountTreeKomperhensifData[] accountsTree;
  private AccountTreeKomperhensifData[] reportElements;
   ...
   */
  public AccountTreeKomperhensif(VariablesSecureApp _vars, ConnectionProvider _conn,
      AccountTreeKomperhensifData[] _accountsTree, AccountTreeKomperhensifData[] _accountsFacts, String _reportNode)
      throws ServletException {
    if (log4j.isDebugEnabled())
      log4j.debug("AccountTreeKomperhensif []");
    vars = _vars;
    conn = _conn;
    ...
    //qty 1 year before
    //reportNodes = new String[2];
    //qty audited
    reportNodes = new String[3];
    ...
    if (reportElements != null && reportElements.length > 0) {
      ...
      AccountTreeKomperhensifData[] operands = AccountTreeKomperhensifData.selectOperands(conn,
          Utility.getContext(conn, vars, "#User_Org", "AccountTreeKomperhensif"),
          Utility.getContext(conn, vars, "#User_Client", "AccountTreeKomperhensif"),
   ...
   */
  public AccountTreeKomperhensif(VariablesSecureApp _vars, ConnectionProvider _conn,
      AccountTreeKomperhensifData[] _accountsTree, AccountTreeKomperhensifData[] _accountsFacts, String[] _reportNodes)
      throws ServletException {
    if (log4j.isDebugEnabled())
      log4j.debug("AccountTreeKomperhensif []");
    vars = _vars;
    ...
    if (reportElements != null && reportElements.length > 0) {
      ...
      reportElements = new AccountTreeKomperhensifData[vec.size()];
   ...
   */
  public AccountTreeKomperhensifData[] getAccounts() {
   ...
   * @return AccountTreeKomperhensifData object with the new element's information.
   */
  private AccountTreeKomperhensifData setDataQty(AccountTreeKomperhensifData reportElement, String isDebitCredit) {
    ...
    for (int i = 0; i < accountsFacts.length; i++) {
      if (accountsFacts[i].id.equals(reportElement.id)) {
        if (isDebitCredit.equals("C")) {
          ...
          //qty 1 year before
          accountsFacts[i].qtyBef = accountsFacts[i].qtycreditBef;
          //qty audited
          accountsFacts[i].qtyAud = accountsFacts[i].qtycreditAud;
        }
        reportElement.qtyOperation = accountsFacts[i].qty;
        reportElement.qtyOperationRef = accountsFacts[i].qtyRef;
        //qty 1 year before
        reportElement.qtyOperationBef = accountsFacts[i].qtyBef;
        //qty audited
        reportElement.qtyOperationAud = accountsFacts[i].qtyAud;
        ...
        //qty 1 year before
        BigDecimal bdQtyBef = new BigDecimal(reportElement.qtyOperationBef);
        //qty audited
        BigDecimal bdQtyAud = new BigDecimal(reportElement.qtyOperationAud);
        ...
        reportElement.qtyRef = (applyShowValueCond(bdQtyRef, reportElement.showvaluecond,
            reportElement.issummary.equals("Y"))).toPlainString();
        //qty 1 year before
        reportElement.qtyBef = (applyShowValueCond(bdQtyBef, reportElement.showvaluecond,
            reportElement.issummary.equals("Y"))).toPlainString();
        //qty audited
        reportElement.qtyAud = (applyShowValueCond(bdQtyAud, reportElement.showvaluecond,
            reportElement.issummary.equals("Y"))).toPlainString();
   ...
   * @return Array of AccountTreeKomperhensifData with the updated tree.
   */
  private AccountTreeKomperhensifData[] updateTreeQuantitiesSign(String rootElement, int level,
      String accountSign) {
    if (accountsTree == null || accountsTree.length == 0)
      return accountsTree;
    AccountTreeKomperhensifData[] result = null;
    Vector<Object> vec = new Vector<Object>();
    // if (log4j.isDebugEnabled())
    // log4j.debug("AccountTreeKomperhensif.updateTreeQuantitiesSign() - elements: " +
    // elements.length);
    if (rootElement == null)
      rootElement = "0";
    for (int i = 0; i < accountsTree.length; i++) {
      if (accountsTree[i].parentId.equals(rootElement)) {
        ...
        AccountTreeKomperhensifData[] dataChilds = updateTreeQuantitiesSign(accountsTree[i].nodeId,
            (level + 1), accountSign);
    ...
    result = new AccountTreeKomperhensifData[vec.size()];
    vec.copyInto(result);
    return result;
  }
   ...
   */
  private boolean hasOperand(String indice, AccountTreeKomperhensifData[] forms) {
    if (indice == null) {
      log4j.error("AccountTreeKomperhensif.hasForm - Missing index");
      return false;
    }
   ...
   */
  private void operandsCalculate(Vector<Object> vecAll, AccountTreeKomperhensifData[] operands,
    ...
    if (log4j.isDebugEnabled())
      log4j.debug("AccountTreeKomperhensif.formsCalculate");
    if (reportElements == null || reportElements.length == 0)
      return;
    if (accountId == null) {
      log4j.error("AccountTreeKomperhensif.formsCalculate - Missing accountId");
      return;
    }
    if (vecTotal == null)
    ...
    //qty 1 year before
    BigDecimal totalBef = new BigDecimal((String) vecTotal.elementAt(2));
    //qty audited
    BigDecimal totalAud = new BigDecimal((String) vecTotal.elementAt(3));
    ...
    for (int i = 0; i < operands.length; i++) {
      if (operands[i].id.equals(accountId)) {
        ...
        for (int j = 0; j < vecAll.size(); j++) {
          AccountTreeKomperhensifData actual = (AccountTreeKomperhensifData) vecAll.elementAt(j);
          log4j.debug("AccountTreeKomperhensif.formsCalculate - actual.nodeId: " + actual.nodeId
          ...
          if (actual.nodeId.equals(operands[i].nodeId)) {
            ...
            actual.qtyRef = (applyShowValueCond(new BigDecimal(actual.qtyOperationRef),
                actual.showvaluecond, actual.issummary.equals("Y"))).toPlainString();
            //qty 1 year before
            actual.qtyBef = (applyShowValueCond(new BigDecimal(actual.qtyOperationBef),
                actual.showvaluecond, actual.issummary.equals("Y"))).toPlainString();
            //qty audited
            actual.qtyAud = (applyShowValueCond(new BigDecimal(actual.qtyOperationAud),
                actual.showvaluecond, actual.issummary.equals("Y"))).toPlainString();
            ...
            totalRef = totalRef.add(new BigDecimal(actual.qtyRef).multiply(new BigDecimal(
                operands[i].sign)));
            //qty 1 year before
            totalBef = totalBef.add(new BigDecimal(actual.qtyBef).multiply(new BigDecimal(
                operands[i].sign)));
            //qty audited
            totalAud = totalAud.add(new BigDecimal(actual.qtyAud).multiply(new BigDecimal(
                operands[i].sign)));
        ...
        /* (b) */if (!encontrado) {
          if (log4j.isDebugEnabled())
            log4j.debug("AccountTreeKomperhensif.formsCalculate - C_ElementValue_ID: " + operands[i].nodeId
                + " not found");
          ...
          //qty 1 year before
          BigDecimal parcialBef = new BigDecimal((String) amounts.elementAt(2));
          //qty audited
          BigDecimal parcialAud = new BigDecimal((String) amounts.elementAt(3));

          if (log4j.isDebugEnabled())
            log4j.debug("AccountTreeKomperhensif.formsCalculate - parcial: " + parcial.toPlainString());
          parcial = parcial.multiply(new BigDecimal(operands[i].sign));
          parcialRef = parcialRef.multiply(new BigDecimal(operands[i].sign));
          //qty 1 year before
          parcialBef = parcialBef.multiply(new BigDecimal(operands[i].sign));
          //qty audited
          parcialAud = parcialAud.multiply(new BigDecimal(operands[i].sign));
          ...
          //qty 1 year before
          totalBef = totalBef.add(parcialBef);
          //qty audited
          totalAud = totalAud.add(parcialAud);
        }
      }
    }
    ...
    //qty 1 year before
    vecTotal.set(2, totalBef.toPlainString());
    //qty audited
    vecTotal.set(3, totalAud.toPlainString());
  }
   ...
   */
  private AccountTreeKomperhensifData[] calculateTree(AccountTreeKomperhensifData[] forms, String[] indice,
   ...
   * @return Array with the new calculated tree.
   */
  private AccountTreeKomperhensifData[] calculateTree(AccountTreeKomperhensifData[] operands, String reportNode,
      Vector<Object> vecTotal) {
  ...
  private AccountTreeKomperhensifData[] calculateTree(AccountTreeKomperhensifData[] operands, String[] reportNode,
    ...
    if (reportNode == null) {
      ...
      //qty 1 year before
      //reportNode = new String[2];
      //qty audited
      reportNode = new String[3];
      reportNode[0] = "0";
    }
    AccountTreeKomperhensifData[] result = null;
    Vector<Object> report = new Vector<Object>();
    if (log4j.isDebugEnabled())
      log4j.debug("AccountTreeKomperhensif.calculateTree() - accounts: " + reportElements.length);
    ...
    //qty 1 year before
    BigDecimal totalBef = new BigDecimal((String) totalAmounts.elementAt(2));
    //qty audited
    BigDecimal totalAud = new BigDecimal((String) totalAmounts.elementAt(3));
    ...
    for (int i = 0; i < reportElements.length; i++) {
      if ((isExactValue && nodeIn(reportElements[i].nodeId, reportNode))
        ...
        AccountTreeKomperhensifData[] reportElementChilds = null;
        ...
        // why????
        {
          ...
          if (!hasOperand(reportElements[i].nodeId, operands)) {
            ...
            //qty 1 year before
            reportElements[i].qtyOperationBef = (new BigDecimal(reportElements[i].qtyOperationBef)
                .add(parcialBef)).toPlainString();
            //qty audited
            reportElements[i].qtyOperationAud = (new BigDecimal(reportElements[i].qtyOperationAud)
                .add(parcialAud)).toPlainString();
          ...
          reportElements[i].qtyRef = (applyShowValueCond(new BigDecimal(
              ...
              .toPlainString();
          //qty 1 year before
          reportElements[i].qtyBef = (applyShowValueCond(new BigDecimal(
              reportElements[i].qtyOperationBef), SVC, reportElements[i].issummary.equals("Y")))
              .toPlainString();
          //qty audited
          reportElements[i].qtyAud = (applyShowValueCond(new BigDecimal(
              reportElements[i].qtyOperationAud), SVC, reportElements[i].issummary.equals("Y")))
              .toPlainString();
          if (resetFlag) {
            reportElements[i].svcreset = "Y";
            reportElements[i].svcresetref = "Y";
            //qty 1 year ref
            reportElements[i].svcresetbef = "Y";
            //qty audited
            reportElements[i].svcresetaud = "Y";
          }
        ...
        if (applysign) {
          ...
          //qty 1 year before
          totalBef = totalBef.add(new BigDecimal(reportElements[i].qtyBef));
          //qty audited
          totalAud = totalAud.add(new BigDecimal(reportElements[i].qtyAud));
        } else {
          ...
          //qty 1 year before
          totalBef = totalBef.add(new BigDecimal(reportElements[i].qtyOperationBef));
          //qty audited
          totalAud = totalAud.add(new BigDecimal(reportElements[i].qtyOperationAud));
        }
    ...
    //qty 1 year before
    totalAmounts.set(2, totalBef.toPlainString());
    //qty audited
    totalAmounts.set(3, totalAud.toPlainString());
    result = new AccountTreeKomperhensifData[report.size()];
   ...
   */
  private AccountTreeKomperhensifData[] levelFilter(String[] indice, boolean found, String strLevel) {
    ...
    Vector<Object> vec = new Vector<Object>();
    if (log4j.isDebugEnabled())
      log4j.debug("AccountTreeKomperhensif.levelFilter() - accounts: " + reportElements.length);
    ...
    for (int i = 0; i < reportElements.length; i++) {
      ...
      if (nodeIn(reportElements[i].parentId, indice)
        ...
        AccountTreeKomperhensifData[] dataChilds = levelFilter(reportElements[i].nodeId,
  ...
  private AccountTreeKomperhensifData[] levelFilter(String indice, boolean found, String strLevel) {
    ...
    i[0] = indice;
    if (log4j.isDebugEnabled())
      log4j.debug("AccountTreeKomperhensif.levelFilter1");
   ...
   */
  public AccountTreeKomperhensifData[] filterStructure(String[] indice, boolean notEmptyLines, String strLevel,
      boolean isLevel) {
    if (log4j.isDebugEnabled())
      log4j.debug("AccountTreeKomperhensif.filterStructure() - accounts: " + reportElements.length);
    if (reportElements == null || reportElements.length == 0)
      return reportElements;
    AccountTreeKomperhensifData[] result = null;
    Vector<Object> vec = new Vector<Object>();

    AccountTreeKomperhensifData[] r = levelFilter(indice, false, strLevel);
    ...
    for (int i = 0; i < r.length; i++) {
      if (r[i].showelement.equals("Y")) {
        ...
        r[i].qtyRef = (applyShowValueCond(new BigDecimal(r[i].qtyRef), r[i].showvaluecond, true))
            .toPlainString();
        //qty 1 year before
        r[i].qtyBef = (applyShowValueCond(new BigDecimal(r[i].qtyBef), r[i].showvaluecond, true))
            .toPlainString();
        //qty audited
        r[i].qtyAud = (applyShowValueCond(new BigDecimal(r[i].qtyAud), r[i].showvaluecond, true))
            .toPlainString();

        if ((!notEmptyLines || (new BigDecimal(r[i].qty).compareTo(BigDecimal.ZERO) != 0 || new BigDecimal(
            r[i].qtyRef).compareTo(BigDecimal.ZERO) != 0 || new BigDecimal(
            r[i].qtyBef).compareTo(BigDecimal.ZERO) != 0 || new BigDecimal(
            r[i].qtyAud).compareTo(BigDecimal.ZERO) != 0)) || "Y".equals(r[i].isalwaysshown)) {
  ...
  public void filterSVC() {
    if (log4j.isDebugEnabled())
      log4j.debug("AccountTreeKomperhensif.filterShowValueCond() - accounts: " + reportElements.length);
    if (reportElements == null || reportElements.length == 0)
      return;
    ...
    for (int i = 0; i < reportElements.length; i++) {
      ...
      //qty 1 year before
      if (reportElements[i].svcresetbef.equals("Y")) {
        levels[2] = Math.min(level, levels[2]);
      }
      //qty audited
      if (reportElements[i].svcresetaud.equals("Y")) {
        levels[3] = Math.min(level, levels[3]);
      }
      if (level > levels[0]) {
      ...
      //qty 1 year before
      if (level > levels[2]) {
        reportElements[i].qtyBef = "0.0";
      }
      if (level == levels[2] && reportElements[i].svcresetbef.equals("N")) {
        levels[2] = Integer.MAX_VALUE;
      }
      //qty audited
      if (level > levels[3]) {
        reportElements[i].qtyAud = "0.0";
      }
      if (level == levels[3] && reportElements[i].svcresetaud.equals("N")) {
        levels[3] = Integer.MAX_VALUE;
      }
    }
  }

  private boolean isAccountLevelLower(String reportAccountLevel, AccountTreeKomperhensifData accountToBeAdded) {
  ...
  private void applySignAsPerParent() {
    ...
    for (AccountTreeKomperhensifData node : accountsTree) {
      parentId = node.id;
