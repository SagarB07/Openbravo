 ...
 */
public class AccountTreeCashflow {
  private static Logger log4j = Logger.getLogger(AccountTreeCashflow.class);
  private VariablesSecureApp vars;
  private ConnectionProvider conn;
  private AccountTreeCashflowData[] accountsFacts;
  private AccountTreeCashflowData[] accountsTree;
  private AccountTreeCashflowData[] reportElements;
   ...
   */
  public AccountTreeCashflow(VariablesSecureApp _vars, ConnectionProvider _conn,
      AccountTreeCashflowData[] _accountsTree, AccountTreeCashflowData[] _accountsFacts, String _reportNode)
      throws ServletException {
    if (log4j.isDebugEnabled())
      log4j.debug("AccountTreeCashflow []");
    vars = _vars;
    conn = _conn;
    ...
    if (reportElements != null && reportElements.length > 0) {
      ...
      AccountTreeCashflowData[] operands = AccountTreeCashflowData.selectOperands(conn,
          Utility.getContext(conn, vars, "#User_Org", "AccountTreeCashflow"),
          Utility.getContext(conn, vars, "#User_Client", "AccountTreeCashflow"),
   ...
   */
  public AccountTreeCashflow(VariablesSecureApp _vars, ConnectionProvider _conn,
      AccountTreeCashflowData[] _accountsTree, AccountTreeCashflowData[] _accountsFacts, String[] _reportNodes)
      throws ServletException {
    if (log4j.isDebugEnabled())
      log4j.debug("AccountTreeCashflow []");
    vars = _vars;
    ...
    if (reportElements != null && reportElements.length > 0) {
      ...
      reportElements = new AccountTreeCashflowData[vec.size()];
   ...
   */
  public AccountTreeCashflowData[] getAccounts() {
   ...
   * @return AccountTreeCashflowData object with the new element's information.
   */
  private AccountTreeCashflowData setDataQty(AccountTreeCashflowData reportElement, String isDebitCredit) {
   ...
   * @return Array of AccountTreeCashflowData with the updated tree.
   */
  private AccountTreeCashflowData[] updateTreeQuantitiesSign(String rootElement, int level,
      String accountSign) {
    if (accountsTree == null || accountsTree.length == 0)
      return accountsTree;
    AccountTreeCashflowData[] result = null;
    Vector<Object> vec = new Vector<Object>();
    // if (log4j.isDebugEnabled())
    // log4j.debug("AccountTreeCashflow.updateTreeQuantitiesSign() - elements: " +
    // elements.length);
    if (rootElement == null)
      rootElement = "0";
    for (int i = 0; i < accountsTree.length; i++) {
      if (accountsTree[i].parentId.equals(rootElement)) {
        ...
        AccountTreeCashflowData[] dataChilds = updateTreeQuantitiesSign(accountsTree[i].nodeId,
            (level + 1), accountSign);
    ...
    result = new AccountTreeCashflowData[vec.size()];
    vec.copyInto(result);
    return result;
  }
   ...
   */
  private boolean hasOperand(String indice, AccountTreeCashflowData[] forms) {
    if (indice == null) {
      log4j.error("AccountTreeCashflow.hasForm - Missing index");
      return false;
    }
   ...
   */
  private void operandsCalculate(Vector<Object> vecAll, AccountTreeCashflowData[] operands,
    ...
    if (log4j.isDebugEnabled())
      log4j.debug("AccountTreeCashflow.formsCalculate");
    if (reportElements == null || reportElements.length == 0)
      return;
    if (accountId == null) {
      log4j.error("AccountTreeCashflow.formsCalculate - Missing accountId");
      return;
    }
    if (vecTotal == null)
    ...
    for (int i = 0; i < operands.length; i++) {
      if (operands[i].id.equals(accountId)) {
        ...
        for (int j = 0; j < vecAll.size(); j++) {
          AccountTreeCashflowData actual = (AccountTreeCashflowData) vecAll.elementAt(j);
          log4j.debug("AccountTreeCashflow.formsCalculate - actual.nodeId: " + actual.nodeId
        ...
        /* (b) */if (!encontrado) {
          if (log4j.isDebugEnabled())
            log4j.debug("AccountTreeCashflow.formsCalculate - C_ElementValue_ID: " + operands[i].nodeId
                + " not found");
          ...
          if (log4j.isDebugEnabled())
            log4j.debug("AccountTreeCashflow.formsCalculate - parcial: " + parcial.toPlainString());
   ...
   */
  private AccountTreeCashflowData[] calculateTree(AccountTreeCashflowData[] forms, String[] indice,
   ...
   * @return Array with the new calculated tree.
   */
  private AccountTreeCashflowData[] calculateTree(AccountTreeCashflowData[] operands, String reportNode,
      Vector<Object> vecTotal) {
  ...
  private AccountTreeCashflowData[] calculateTree(AccountTreeCashflowData[] operands, String[] reportNode,
    ...
    if (log4j.isDebugEnabled())
      log4j.debug("AccountTreeCashflow.calculateTree() - accounts: " + reportElements.length);
    ...
    for (int i = 0; i < reportElements.length; i++) {
      if ((isExactValue && nodeIn(reportElements[i].nodeId, reportNode))
        ...
        AccountTreeCashflowData[] reportElementChilds = null;
    ...
    //qty 1 year before
    //totalAmounts.set(2, totalRef.toPlainString());

    result = new AccountTreeCashflowData[report.size()];
   ...
   */
  private AccountTreeCashflowData[] levelFilter(String[] indice, boolean found, String strLevel) {
    ...
    Vector<Object> vec = new Vector<Object>();
    if (log4j.isDebugEnabled())
      log4j.debug("AccountTreeCashflow.levelFilter() - accounts: " + reportElements.length);
    ...
    for (int i = 0; i < reportElements.length; i++) {
      ...
      if (nodeIn(reportElements[i].parentId, indice)
        ...
        AccountTreeCashflowData[] dataChilds = levelFilter(reportElements[i].nodeId,
  ...
  private AccountTreeCashflowData[] levelFilter(String indice, boolean found, String strLevel) {
    ...
    i[0] = indice;
    if (log4j.isDebugEnabled())
      log4j.debug("AccountTreeCashflow.levelFilter1");
   ...
   */
  public AccountTreeCashflowData[] filterStructure(String[] indice, boolean notEmptyLines, String strLevel,
      boolean isLevel) {
    if (log4j.isDebugEnabled())
      log4j.debug("AccountTreeCashflow.filterStructure() - accounts: " + reportElements.length);
    if (reportElements == null || reportElements.length == 0)
      return reportElements;
    AccountTreeCashflowData[] result = null;
    Vector<Object> vec = new Vector<Object>();

    AccountTreeCashflowData[] r = levelFilter(indice, false, strLevel);
  ...
  public void filterSVC() {
    if (log4j.isDebugEnabled())
      log4j.debug("AccountTreeCashflow.filterShowValueCond() - accounts: " + reportElements.length);
    if (reportElements == null || reportElements.length == 0)
      return;
  ...
  private boolean isAccountLevelLower(String reportAccountLevel, AccountTreeCashflowData accountToBeAdded) {
  ...
  private void applySignAsPerParent() {
    ...
    for (AccountTreeCashflowData node : accountsTree) {
      parentId = node.id;
