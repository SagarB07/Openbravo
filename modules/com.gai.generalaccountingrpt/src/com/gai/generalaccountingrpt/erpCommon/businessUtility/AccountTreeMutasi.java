 ...
 */
public class AccountTreeMutasi {
  private static Logger log4j = Logger.getLogger(AccountTreeMutasi.class);
  private VariablesSecureApp vars;
  private ConnectionProvider conn;
  private AccountTreeMutasiData[] accountsFacts;
  private AccountTreeMutasiData[] accountsTree;
  private AccountTreeMutasiData[] reportElements;
   ...
   */
  public AccountTreeMutasi(VariablesSecureApp _vars, ConnectionProvider _conn,
      AccountTreeMutasiData[] _accountsTree, AccountTreeMutasiData[] _accountsFacts, String _reportNode)
      throws ServletException {
    if (log4j.isDebugEnabled())
      log4j.debug("AccountTreeMutasi []");
    vars = _vars;
    conn = _conn;
    ...
    //reportElements = getMonthAcct(accountsFacts);
    ...
    if (reportElements != null && reportElements.length > 0) {
      ...
      AccountTreeMutasiData[] operands = AccountTreeMutasiData.selectOperands(conn,
          Utility.getContext(conn, vars, "#User_Org", "AccountTreeMutasi"),
          Utility.getContext(conn, vars, "#User_Client", "AccountTreeMutasi"),
   ...
   */
  public AccountTreeMutasi(VariablesSecureApp _vars, ConnectionProvider _conn,
      AccountTreeMutasiData[] _accountsTree, AccountTreeMutasiData[] _accountsFacts, String[] _reportNodes)
      throws ServletException {
    if (log4j.isDebugEnabled())
      log4j.debug("AccountTreeMutasi []");
    vars = _vars;
    ...
    /*for (int i = 0; i < accountsFacts.length; i++) {
      log4j.debug("Month Acct: " +reportElements[i].monthacct);
      reportElements[i].monthacct = accountsFacts[i].monthacct;
    }*/
    ...
    if (reportElements != null && reportElements.length > 0) {
      ...
      reportElements = new AccountTreeMutasiData[vec.size()];
   ...
   */
  public AccountTreeMutasiData[] getAccounts() {
    log4j.debug("Masuk Return getAccounts");
   ...
   * @return AccountTreeMutasiData object with the new element's information.
   */
  private AccountTreeMutasiData setDataQty(AccountTreeMutasiData reportElement, String isDebitCredit) {
    ...
    for (int i = 0; i < accountsFacts.length; i++) {
      if (accountsFacts[i].id.equals(reportElement.id)) {
        ...
        //test
        //reportElement.quantity = accountsFacts[i].quantity;
        //mutasi
        //reportElement.monthacct = accountsFacts[i].monthacct;
        //end mutasi
        ...
        //test
        //BigDecimal bdQuantity = new BigDecimal(reportElement.quantity);
        ...
        reportElement.qtyRef = (applyShowValueCond(bdQtyRef, reportElement.showvaluecond,
            reportElement.issummary.equals("Y"))).toPlainString();
        //test
        //reportElement.quantity = (applyShowValueCond(bdQuantity, reportElement.showvaluecond,
        //    reportElement.issummary.equals("Y"))).toPlainString(); 
  ...
  //mutasi
  private AccountTreeMutasiData[] getMonthAcct(AccountTreeMutasiData[] acctMonth) {
    //AccountTreeMutasiData[] reportElement;

    if (acctMonth == null || acctMonth.length == 0)
      return reportElements;
    
    for (int i = 0; i < acctMonth.length; i++) {
      reportElements[i].monthacct = acctMonth[i].monthacct;
    }
    return reportElements;
  }
  //end mutasi
   ...
   * @return Array of AccountTreeMutasiData with the updated tree.
   */
  private AccountTreeMutasiData[] updateTreeQuantitiesSign(String rootElement, int level,
      String accountSign) {
    if (accountsTree == null || accountsTree.length == 0)
      return accountsTree;
    AccountTreeMutasiData[] result = null;
    Vector<Object> vec = new Vector<Object>();
    // if (log4j.isDebugEnabled())
    // log4j.debug("AccountTreeMutasi.updateTreeQuantitiesSign() - elements: " +
    // elements.length);
    if (rootElement == null)
      rootElement = "0";
    for (int i = 0; i < accountsTree.length; i++) {
      if (accountsTree[i].parentId.equals(rootElement)) {
        ...
        AccountTreeMutasiData[] dataChilds = updateTreeQuantitiesSign(accountsTree[i].nodeId,
            (level + 1), accountSign);
        accountsTree[i].elementLevel = Integer.toString(level);
        accountsTree[i] = setDataQty(accountsTree[i], accountsTree[i].accountsign);
        //mutasi
        accountsTree[i].monthacct = accountsTree[i].monthacct;
        //end
    ...
    result = new AccountTreeMutasiData[vec.size()];
    vec.copyInto(result);
    return result;
  }
   ...
   */
  private boolean hasOperand(String indice, AccountTreeMutasiData[] forms) {
    if (indice == null) {
      log4j.error("AccountTreeMutasi.hasForm - Missing index");
      return false;
    }
   ...
   */
  private void operandsCalculate(Vector<Object> vecAll, AccountTreeMutasiData[] operands,
    ...
    if (log4j.isDebugEnabled())
      log4j.debug("AccountTreeMutasi.formsCalculate");
    if (reportElements == null || reportElements.length == 0)
      return;
    if (accountId == null) {
      log4j.error("AccountTreeMutasi.formsCalculate - Missing accountId");
      return;
    }
    if (vecTotal == null)
    ...
    //test
    //BigDecimal totalQty = new BigDecimal((String) vecTotal.elementAt(2));
    ...
    for (int i = 0; i < operands.length; i++) {
      if (operands[i].id.equals(accountId)) {
        ...
        for (int j = 0; j < vecAll.size(); j++) {
          AccountTreeMutasiData actual = (AccountTreeMutasiData) vecAll.elementAt(j);
          log4j.debug("AccountTreeMutasi.formsCalculate - actual.nodeId: " + actual.nodeId
          ...
          if (actual.nodeId.equals(operands[i].nodeId)) {
            ...
            actual.qtyRef = (applyShowValueCond(new BigDecimal(actual.qtyOperationRef),
                actual.showvaluecond, actual.issummary.equals("Y"))).toPlainString();
            //test
            //actual.quantity = (applyShowValueCond(new BigDecimal(actual.quantity),
            //    actual.showvaluecond, actual.issummary.equals("Y"))).toPlainString();            
            ...
            totalRef = totalRef.add(new BigDecimal(actual.qtyRef).multiply(new BigDecimal(
                operands[i].sign)));
            //test
            //totalQty = totalQty.add(new BigDecimal(actual.quantity).multiply(new BigDecimal(
            //    operands[i].sign)));
            break;
          }
        }
        /* (b) */if (!encontrado) {
          if (log4j.isDebugEnabled())
            log4j.debug("AccountTreeMutasi.formsCalculate - C_ElementValue_ID: " + operands[i].nodeId
                + " not found");
          ...
          //test
          //BigDecimal parcialQty = new BigDecimal((String) amounts.elementAt(2));

          if (log4j.isDebugEnabled())
            log4j.debug("AccountTreeMutasi.formsCalculate - parcial: " + parcial.toPlainString());
          parcial = parcial.multiply(new BigDecimal(operands[i].sign));
          parcialRef = parcialRef.multiply(new BigDecimal(operands[i].sign));
          //test
          //parcialQty = parcialQty.multiply(new BigDecimal(operands[i].sign));
          ...
          //test
          //totalQty = totalQty.add(parcialQty);
        }
      }
    }
    ...
    //TEST
    //vecTotal.set(2, totalQty.toPlainString());
  }
   ...
   */
  private AccountTreeMutasiData[] calculateTree(AccountTreeMutasiData[] forms, String[] indice,
   ...
   * @return Array with the new calculated tree.
   */
  private AccountTreeMutasiData[] calculateTree(AccountTreeMutasiData[] operands, String reportNode,
      Vector<Object> vecTotal) {
  ...
  private AccountTreeMutasiData[] calculateTree(AccountTreeMutasiData[] operands, String[] reportNode,
    ...
    //test
    //BigDecimal totalQty = new BigDecimal((String) totalAmounts.elementAt(2));
    ...
    for (int i = 0; i < reportElements.length; i++) {
      if ((isExactValue && nodeIn(reportElements[i].nodeId, reportNode))
        ...
        AccountTreeMutasiData[] reportElementChilds = null;
        ...
        // why????
        {
          ...
          if (!hasOperand(reportElements[i].nodeId, operands)) {
            ...
            //test
            //reportElements[i].quantity = (new BigDecimal(reportElements[i].quantity)
            //    .add(parcialQty)).toPlainString();

            reportElements[i].monthacct = reportElements[i].monthacct;
        ...
        if (applysign) {  
          total = total.add(new BigDecimal(reportElements[i].qty)==null ? BigDecimal.ZERO : new BigDecimal(reportElements[i].qty));
          totalRef = totalRef.add(new BigDecimal(reportElements[i].qtyRef)==null ? BigDecimal.ZERO : new BigDecimal(reportElements[i].qtyRef));
          //test
          //totalQty = totalQty.add(new BigDecimal(reportElements[i].quantity)==null? BigDecimal.ZERO : new BigDecimal(reportElements[i].quantity));
        } else {
          total = total.add(new BigDecimal(reportElements[i].qtyOperation)==null ? BigDecimal.ZERO : new BigDecimal(reportElements[i].qtyOperation));
          totalRef = totalRef.add(new BigDecimal(reportElements[i].qtyOperationRef)==null ? BigDecimal.ZERO : new BigDecimal(reportElements[i].qtyOperationRef));
          //test
          //totalQty = totalQty.add(new BigDecimal(reportElements[i].quantity)==null ? BigDecimal.ZERO : new BigDecimal(reportElements[i].quantity));
        }
        ...
        ElementValue repElementAccount = OBDal.getInstance().get(ElementValue.class,
            reportElements[i].id);
        BigDecimal qtyOperation = new BigDecimal(reportElements[i].qtyOperation) == null ? BigDecimal.ZERO : new BigDecimal(reportElements[i].qtyOperation);
    ...
    //test
    //totalAmounts.set(2, totalQty.toPlainString());
    result = new AccountTreeMutasiData[report.size()];
   ...
   */
  private AccountTreeMutasiData[] levelFilter(String[] indice, boolean found, String strLevel) {
    ...
    for (int i = 0; i < reportElements.length; i++) {
      ...
      if (nodeIn(reportElements[i].parentId, indice)
        ...
        AccountTreeMutasiData[] dataChilds = levelFilter(reportElements[i].nodeId,
   ...
   */
  public AccountTreeMutasiData[] filterStructure(String[] indice, boolean notEmptyLines, String strLevel,
      boolean isLevel) {
    if (log4j.isDebugEnabled())
    ...
    AccountTreeMutasiData[] r = levelFilter(indice, false, strLevel);
    ...
    for (int i = 0; i < r.length; i++) {
      if (r[i].showelement.equals("Y")) {
        ...
        r[i].qtyRef = (applyShowValueCond(new BigDecimal(r[i].qtyRef), r[i].showvaluecond, true))
            .toPlainString();
        //test
        //r[i].quantity = (applyShowValueCond(new BigDecimal(r[i].quantity), r[i].showvaluecond, true))
        //    .toPlainString();            
  ...
  private boolean isAccountLevelLower(String reportAccountLevel, AccountTreeMutasiData accountToBeAdded) {
  ...
  private void applySignAsPerParent() {
    ...
    for (AccountTreeMutasiData node : accountsTree) {
      parentId = node.id;
