package com.gai.umk.actionHandler;
...
public class GumkHandlerDocNo extends EntityPersistenceEventObserver {
	...
	private void handleEvent(EntityPersistenceEvent event) {
		...
		if (getWindowId.equals("backgroundProcess")) {
			...
			if (!isUpdate) {
				...
				if (getWindowId.equals("8DC5FDE27DB846BF83FFA7A5EE6A3A9E")) {
					log4j.info("Conditional 1(UMK)");
				...
				else {//update terjadi di window selain kondisi ke 1 {UMK}
					log4j.info("Conditional 3(selain UMK)");
	...
	private static final Entity imEntity = ModelProvider.getInstance().getEntity(GumkUmk.ENTITY_NAME);
	private static final Property imDocumentNoProperty = imEntity.getProperty(GumkUmk.PROPERTY_DOCUMENTNO);
	private static final Property imgaiDocTypeTargetProperty = imEntity.getProperty(GumkUmk.PROPERTY_DOCTYPE);
	private static final Entity[] entities = { imEntity };

	private static final Logger log4j = Logger.getLogger(GumkHandlerDocNo.class);
