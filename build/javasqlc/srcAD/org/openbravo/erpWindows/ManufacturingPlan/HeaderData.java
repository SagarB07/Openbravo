 ...
 */
  public static HeaderData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN MRP_Run_Production.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Value), ''))),'') ) END) AS AD_Org_IDR, " +
 ...
 */
  ...
  public int update(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        SET AD_Org_ID = (?) , DateDoc = TO_DATE(?) , Name = (?) , Timehorizon = TO_NUMBER(?) , Securitymargin = TO_NUMBER(?) , MRP_Planner_ID = (?) , C_BPartner_ID = (?) , C_BP_Group_ID = (?) , M_Product_ID = (?) , M_Product_Category_ID = (?) , Launchwr = (?) , Launchmr = (?) , Simulate = (?) , Recalculatestock = (?) , MRP_Run_Production_ID = (?) , Description = (?) , IsActive = (?) , AD_Client_ID = (?) , updated = now(), updatedby = ? " +
  ...
  public int insert(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        (AD_Org_ID, DateDoc, Name, Timehorizon, Securitymargin, MRP_Planner_ID, C_BPartner_ID, C_BP_Group_ID, M_Product_ID, M_Product_Category_ID, Launchwr, Launchmr, Simulate, Recalculatestock, MRP_Run_Production_ID, Description, IsActive, AD_Client_ID, created, createdby, updated, updatedBy)" +
