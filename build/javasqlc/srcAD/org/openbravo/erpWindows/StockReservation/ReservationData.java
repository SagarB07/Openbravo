 ...
 */
  public static ReservationData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN M_Reservation.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Value), ''))),'') ) END) AS AD_Org_IDR, " +
 ...
 */
  ...
  public int update(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        SET RES_Process = (?) , AD_Org_ID = (?) , M_Product_ID = (?) , Quantity = TO_NUMBER(?) , C_Uom_ID = (?) , C_Orderline_ID = (?) , M_Warehouse_ID = (?) , M_Locator_ID = (?) , M_Attributesetinstance_ID = (?) , Reservedgoodmnt_Pe = (?) , Managereservation_Pe = (?) , RES_Status = (?) , ReleasedQty = TO_NUMBER(?) , Reservedqty = TO_NUMBER(?) , Isactive = (?) , M_Reservation_ID = (?) , AD_Client_ID = (?) , updated = now(), updatedby = ? " +
  ...
  public int insert(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        (RES_Process, AD_Org_ID, M_Product_ID, Quantity, C_Uom_ID, C_Orderline_ID, M_Warehouse_ID, M_Locator_ID, M_Attributesetinstance_ID, Reservedgoodmnt_Pe, Managereservation_Pe, RES_Status, ReleasedQty, Reservedqty, Isactive, M_Reservation_ID, AD_Client_ID, created, createdby, updated, updatedBy)" +
