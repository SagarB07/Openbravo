 ...
 */
  public static WorkEffortData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        ? AS LANGUAGE " +
      "        FROM M_Production left join (select AD_Org_ID, Value from AD_Org) table1 on (M_Production.AD_Org_ID = table1.AD_Org_ID) left join ad_ref_list_v list1 on (list1.ad_reference_id = '234' and list1.ad_language = ?  AND M_Production.Posted = TO_CHAR(list1.value))" +
      "        WHERE 2=2 " +
 ...
 */
  ...
  public int update(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      "        UPDATE M_Production" +
      "        SET AD_Org_ID = (?) , MovementDate = TO_DATE(?) , Starttime = TO_TIMESTAMP(?,'HH24:MI:SS') , Endtime = TO_TIMESTAMP(?,'HH24:MI:SS') , DocumentNo = (?) , Validating = (?) , Processed = (?) , Posted = (?) , IsActive = (?) , IsSOTrx = (?) , AD_Client_ID = (?) , M_Production_ID = (?) , updated = now(), updatedby = ? " +
  ...
  public int insert(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        (AD_Org_ID, MovementDate, Starttime, Endtime, DocumentNo, Validating, Processed, Posted, IsActive, IsSOTrx, AD_Client_ID, M_Production_ID, created, createdby, updated, updatedBy)" +
