 ...
 */
  public static HeaderData[] selectEdit(ConnectionProvider connectionProvider, String dateTimeFormat, String paramLanguage, String glJournalbatchId, String key, String adUserClient, String adOrgClient, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "(CASE WHEN GL_Journal.AD_Org_ID IS NULL THEN '' ELSE  (COALESCE(TO_CHAR(TO_CHAR(COALESCE(TO_CHAR(table1.Value), ''))),'') ) END) AS AD_Org_IDR, " +
 ...
 */
  public static String selectDef1634(ConnectionProvider connectionProvider, String C_PERIOD_ID, String DateDoc)    throws ServletException {
    ...
    st = connectionProvider.getPreparedStatement(strSql);
      ...
      result = st.executeQuery();
      if(result.next()) {
        strReturn = UtilSql.getValue(result, "COALESCE((SELECTENDDATEFROMC_PERIODWHEREC_PERIOD_ID=:1ANDPERIODTYPE='A'),TO_DATE(:2))");
      }
      result.close();
    } catch(SQLException e){
      log4j.error("SQL error in query: " + strSql + "Exception:"+ e);
 ...
 */
  public static String selectDef1636(ConnectionProvider connectionProvider, String C_Period_ID, String AD_Client_ID, String AD_Org_ID, String DateAcct)    throws ServletException {
    ...
    st = connectionProvider.getPreparedStatement(strSql);
      ...
      result = st.executeQuery();
      if(result.next()) {
        strReturn = UtilSql.getValue(result, "CASEWHEN(:1<>''AND:2ISNOTNULL)THEN:3ELSEC_PERIOD_IDEND");
      }
      result.close();
    } catch(SQLException e){
      log4j.error("SQL error in query: " + strSql + "Exception:"+ e);
 ...
 */
  ...
  public int update(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      "        UPDATE GL_Journal" +
      "        SET AD_Org_ID = (?) , C_AcctSchema_ID = (?) , DocumentNo = (?) , Description = (?) , C_DocType_ID = (?) , DateDoc = TO_DATE(?) , DateAcct = TO_DATE(?) , C_Period_ID = (?) , C_Currency_ID = (?) , CurrencyRateType = (?) , CurrencyRate = TO_NUMBER(?) , IsOpening = (?) , ControlAmt = TO_NUMBER(?) , TotalDr = TO_NUMBER(?) , DocAction = (?) , TotalCr = TO_NUMBER(?) , DocStatus = (?) , Posted = (?) , IsActive = (?) , IsPrinted = (?) , GL_Category_ID = (?) , IsApproved = (?) , C_Bpartner_ID = (?) , M_Product_ID = (?) , C_Project_ID = (?) , C_Costcenter_ID = (?) , A_Asset_ID = (?) , C_Campaign_ID = (?) , User1_ID = (?) , User2_ID = (?) , PostingType = (?) , AD_Client_ID = (?) , GL_JournalBatch_ID = (?) , Processing = (?) , Processed = (?) , GL_Journal_ID = (?) , updated = now(), updatedby = ? " +
      "        WHERE GL_Journal.GL_Journal_ID = ? " +
  ...
  public int insert(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        (AD_Org_ID, C_AcctSchema_ID, DocumentNo, Description, C_DocType_ID, DateDoc, DateAcct, C_Period_ID, C_Currency_ID, CurrencyRateType, CurrencyRate, IsOpening, ControlAmt, TotalDr, DocAction, TotalCr, DocStatus, Posted, IsActive, IsPrinted, GL_Category_ID, IsApproved, C_Bpartner_ID, M_Product_ID, C_Project_ID, C_Costcenter_ID, A_Asset_ID, C_Campaign_ID, User1_ID, User2_ID, PostingType, AD_Client_ID, GL_JournalBatch_ID, Processing, Processed, GL_Journal_ID, created, createdby, updated, updatedBy)" +
