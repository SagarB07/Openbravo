 ...
 */
  public static String selectDef5661DEEC430CFC21E040007F01004C04(ConnectionProvider connectionProvider)    throws ServletException {
    ...
    st = connectionProvider.getPreparedStatement(strSql);
      ...
      result = st.executeQuery();
      if(result.next()) {
        strReturn = UtilSql.getValue(result, "MAX(AD_MODULE_ID)");
      }
      result.close();
    } catch(SQLException e){
      log4j.error("SQL error in query: " + strSql + "Exception:"+ e);
 ...
 */
  ...
  public int update(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      "        UPDATE AD_Window" +
      "        SET AD_Module_ID = (?) , Name = (?) , WindowType = (?) , Description = (?) , Help = (?) , IsSOTrx = (?) , Isthreadsafe = (?) , Processing = (?) , IsAdvancedFeature = (?) , IsActive = (?) , AD_Client_ID = (?) , AD_Image_ID = (?) , IsDefault = (?) , AD_Window_ID = (?) , AD_Org_ID = (?) , updated = now(), updatedby = ? " +
  ...
  public int insert(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        (AD_Module_ID, Name, WindowType, Description, Help, IsSOTrx, Isthreadsafe, Processing, IsAdvancedFeature, IsActive, AD_Client_ID, AD_Image_ID, IsDefault, AD_Window_ID, AD_Org_ID, created, createdby, updated, updatedBy)" +
      "        VALUES ((?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), (?), now(), ?, now(), ?)";
