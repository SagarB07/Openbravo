...
package com.gai.trialbalancerpt.erpCommon.ad_reports;
...
static Logger log4j = Logger.getLogger(ReportTrialBalanceData.class);
  ...
  public String saldoInicialDebet;
  public String saldoInicialCredit;
  public String saldoFinalDebet;
  public String saldoFinalCredit;
  ...
  public String getField(String fieldName) {
    if (fieldName.equalsIgnoreCase("PARENT_ID") || fieldName.equals("parentId"))
      ...
      return pjname;
    else if (fieldName.equalsIgnoreCase("SALDO_INICIAL_DEBET") || fieldName.equals("saldoInicialDebet"))
      return saldoInicialDebet;
    else if (fieldName.equalsIgnoreCase("SALDO_INICIAL_CREDIT") || fieldName.equals("saldoInicialCredit"))
      return saldoInicialCredit;
    else if (fieldName.equalsIgnoreCase("SALDO_FINAL_DEBET") || fieldName.equals("saldoFinalDebet"))
      return saldoFinalDebet;
    else if (fieldName.equalsIgnoreCase("SALDO_FINAL_CREDIT") || fieldName.equals("saldoFinalCredit"))
      return saldoFinalCredit;
  ...
  public static ReportTrialBalanceData[] select(ConnectionProvider connectionProvider, String dateFrom, String dateTo, String org, String treeAcct, String acctSchema, String factaccttype, String orgFamily, String adUserClient, String adUserOrg, String parDateFrom, String parDateTo, String accountFrom, String accountTo, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "      0 AS SALDO_INICIAL_DEBET, 0 AS SALDO_INICIAL_CREDIT," +
      "      0 AS SALDO_FINAL_DEBET, 0 AS SALDO_FINAL_CREDIT, '' AS ACCOUNTSIGN" +
      "      FROM" +
    ...
    strSql = strSql + 
      ...
      "      AND F.ISACTIVE = 'Y'" +
      "      GROUP BY ACCOUNT_ID,EV.NAME, EV.VALUE, EV.ELEMENTLEVEL, ACCOUNTSIGN) A" +
    ...
    st = connectionProvider.getPreparedStatement(strSql);
      ...
      while(continueResult && result.next()) {
        ...
        objectReportTrialBalanceData.saldoInicialDebet = UtilSql.getValue(result, "SALDO_INICIAL_DEBET");
        objectReportTrialBalanceData.saldoInicialCredit = UtilSql.getValue(result, "SALDO_INICIAL_CREDIT");
        objectReportTrialBalanceData.saldoFinalDebet = UtilSql.getValue(result, "SALDO_FINAL_DEBET");
        objectReportTrialBalanceData.saldoFinalCredit = UtilSql.getValue(result, "SALDO_FINAL_CREDIT");
  ...
  public static ReportTrialBalanceData[] set()    throws ServletException {
    ...
    objectReportTrialBalanceData[0].saldoInicialDebet = "";
    objectReportTrialBalanceData[0].saldoInicialCredit = "";
    objectReportTrialBalanceData[0].saldoFinalDebet = "";
    objectReportTrialBalanceData[0].saldoFinalCredit = "";
  ...
  public static ReportTrialBalanceData[] selectAccountLines(ConnectionProvider connectionProvider, String groupby, String adLanguage, String accountLevel, String orgFamily, String adUserClient, String adUserOrg, String accountFrom, String accountTo, String parDateFrom, String accountId, String cBpartnerId, String mProductId, String cProjectId, String acctSchema, String factaccttype, String parDateTo, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "              (CASE WHEN ACCOUNTSIGN = 'D' THEN SUM(SALDO_INICIAL) ELSE 0 END) AS SALDO_INICIAL_DEBET, " +
      "              (CASE WHEN ACCOUNTSIGN = 'C' THEN " +
      "              	(CASE WHEN SUM(SALDO_INICIAL) < 0 THEN ABS(SUM(SALDO_INICIAL)) ELSE SUM(SALDO_INICIAL) END )" +
      "              ELSE 0 END) AS SALDO_INICIAL_CREDIT," +
      "              SUM(AMTACCTDR) AS AMTACCTDR," +
      "              SUM(AMTACCTCR) AS AMTACCTCR," +
      "              SUM(SALDO_INICIAL+AMTACCTDR-AMTACCTCR) AS SALDO_FINAL," +
      "              (CASE WHEN ACCOUNTSIGN = 'D' THEN SUM(SALDO_INICIAL+AMTACCTDR-AMTACCTCR) ELSE 0 END) AS SALDO_FINAL_DEBET," +
      "              (CASE WHEN ACCOUNTSIGN = 'C' THEN " +
      "              	(CASE WHEN SUM(SALDO_INICIAL+AMTACCTDR-AMTACCTCR) < 0 THEN ABS(SUM(SALDO_INICIAL+AMTACCTDR-AMTACCTCR)) ELSE SUM(SALDO_INICIAL+AMTACCTDR-AMTACCTCR)  END)" +
      "              ELSE 0 END) AS SALDO_FINAL_CREDIT," +
      "              groupbyid," +
      "              CASE ?" +
      "                WHEN 'BPartner' THEN " +
      ...
      "                  groupbyname, groupbyid, 0 AS SALDO_INICIAL_DEBET, 0 AS SALDO_INICIAL_CREDIT, ACCOUNTSIGN" +
      "           FROM" +
      ...
      "                       ELSE '' END AS groupbyname, ACCOUNTSIGN" +
    ...
    strSql = strSql + 
      "                    AND F.ISACTIVE = 'Y'" +
      "                    )) A" +
      "           GROUP BY ACCOUNT_ID, ID, groupbyname, groupbyid, NAME, ACCOUNTSIGN" +
      "           HAVING SUM(AMTACCTDR) - SUM(AMTACCTCR) <> 0 )" +
      "           UNION" +
      "           (SELECT ID, ACCOUNT_ID, NAME," +
      "                   SUM(AMTACCTDR) AS AMTACCTDR," +
      "                   SUM(AMTACCTCR) AS AMTACCTCR," +
      "                   0 AS SALDO_INICIAL, groupbyname, groupbyid, 0 AS SALDO_INICIAL_DEBET, 0 AS SALDO_INICIAL_CREDIT, ACCOUNTSIGN" +
    ...
    strSql = strSql + 
      ...
      "       GROUP BY ACCOUNT_ID, ID, groupbyid, groupbyname, NAME, ACCOUNTSIGN" +
  ...
  public static ReportTrialBalanceData[] selectXLS(ConnectionProvider connectionProvider, String adLanguage, String accountLevel, String orgFamily, String adUserClient, String adUserOrg, String accountFrom, String accountTo, String parDateFrom, String cBpartnerId, String mProductId, String cProjectId, String acctSchema, String factaccttype, String parDateTo, int firstRegister, int numberRegisters)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "             END AS pjname, " +
      "             (CASE WHEN ACCOUNTSIGN = 'D' THEN COALESCE(SUM(SALDO_INICIAL), 0) ELSE 0 END) AS SALDO_INICIAL_DEBET, " +
      "             (CASE WHEN ACCOUNTSIGN = 'C' THEN " +
      "             	(CASE WHEN SUM(SALDO_INICIAL) < 0 THEN ABS(SUM(SALDO_INICIAL)) ELSE SUM(SALDO_INICIAL) END ) " +
      "              ELSE 0 END)AS SALDO_INICIAL_CREDIT," +
      "             (CASE WHEN ACCOUNTSIGN = 'D' THEN SUM(SALDO_INICIAL+AMTACCTDR-AMTACCTCR) ELSE 0 END) AS SALDO_FINAL_DEBET," +
      ...
      "                   0 AS AMTACCTDR, 0 AS AMTACCTCR, " +
      "                   COALESCE(SUM(AMTACCTDR-AMTACCTCR), 0) AS SALDO_INICIAL," +
      "                   bpid, pdid, pjid, bpname, pdname, 0 AS SALDO_INICIAL_DEBET, 0 AS SALDO_INICIAL_CREDIT, ACCOUNTSIGN" +
      "          FROM" +
      "              ((SELECT F.ACCOUNT_ID AS ID, EV.VALUE AS ACCOUNT_ID, EV.NAME AS NAME," +
      "                      F.AMTACCTDR, F.AMTACCTCR, F.FACTACCTTYPE, F.DATEACCT," +
      "                      c_bpartner.c_bpartner_id as bpid, m_product.m_product_id as pdid, c_project.c_project_id as pjid," +
      "                      c_bpartner.name as bpname, m_product.name as pdname, ACCOUNTSIGN" +
      "               FROM C_ELEMENTVALUE EV, FACT_ACCT F" +
      "                    LEFT JOIN C_BPARTNER ON f.C_BPARTNER_ID = C_BPARTNER.C_BPARTNER_ID" +
      "                    LEFT JOIN M_PRODUCT ON f.M_PRODUCT_ID = M_PRODUCT.M_PRODUCT_ID" +
      "                    LEFT JOIN C_PROJECT ON f.C_PROJECT_ID = C_PROJECT.C_PROJECT_ID" +
    ...
    strSql = strSql + 
      "                     AND F.ISACTIVE = 'Y'" +
      "                     )) A" +
      "          GROUP BY ACCOUNT_ID, ID, bpid, pdid, pjid, bpname, pdname, NAME, ACCOUNTSIGN" +
      "          HAVING SUM(AMTACCTDR) - SUM(AMTACCTCR) <> 0 )" +
      "          UNION" +
      "          (SELECT ID, ACCOUNT_ID, NAME," +
      "                  SUM(AMTACCTDR) AS AMTACCTDR," +
      "                  SUM(AMTACCTCR) AS AMTACCTCR," +
      "                  0 AS SALDO_INICIAL, bpid, pdid, pjid, bpname, pdname, 0 AS SALDO_INICIAL_DEBET, 0 AS SALDO_INICIAL_CREDIT, ACCOUNTSIGN" +
      "           FROM" +
      "               (SELECT F.ACCOUNT_ID AS ID, EV.VALUE AS ACCOUNT_ID, EV.NAME AS NAME," +
      "                       F.AMTACCTDR, F.AMTACCTCR, F.FACTACCTTYPE," +
      "                       c_bpartner.c_bpartner_id as bpid, m_product.m_product_id as pdid, c_project.c_project_id as pjid," +
    ...
    strSql = strSql + 
      ...
      "                      AND F.FACTACCTTYPE <> 'C'" +
      "                      AND F.ISACTIVE = 'Y') B" +
      "           GROUP BY ACCOUNT_ID, ID, NAME, bpname, bpid, pdname, pdid, pjid, ACCOUNTSIGN )) C" +
      "      GROUP BY ACCOUNT_ID, ID, NAME, bpname, bpid, pdname, pdid, pjid, ACCOUNTSIGN" +
  ...
  public static String selectSubaccountDescription(ConnectionProvider connectionProvider, String elementValueId)    throws ServletException {
    ...
    st = connectionProvider.getPreparedStatement(strSql);
      ...
      result = st.executeQuery();
      if(result.next()) {
        strReturn = UtilSql.getValue(result, "COALESCE(TO_CHAR(NAME),TO_CHAR(''))");
      }
      result.close();
    } catch(SQLException e){
      log4j.error("SQL error in query: " + strSql + "Exception:"+ e);
