...
public class DocInvoice extends AcctServer {
  ...
  private DocLine[] loadLines() {
    ...
    for (int i = 0; i < data.length; i++) {
      ...
      String ProductId = data[i].mProductId;
  ...
  public Fact createFact(AcctSchema as, ConnectionProvider conn, Connection con,
    ...
    //added by GAI Team
    DocInvoiceData[] isCustom = DocInvoiceData.selectIsCustom(conn, AD_Client_ID);
    String isSales = isCustom[0].issales;
    String isPurchase = isCustom[0].ispurchase;
    ...
    if (DocumentType.equals(AcctServer.DOCTYPE_ARInvoice)
      ...
      if (isSales.equals("N")){
        if (m_payments == null || m_payments.length == 0) 
          for (int i = 0; m_debt_payments != null && i < m_debt_payments.length; i++) {
            log4jDocInvoice.debug("Masuk ke kondisi m_payments == null");
          ...
          for (int i = 0; m_payments != null && i < m_payments.length; i++) {
            log4jDocInvoice.debug("ini Payment : "+m_payments[i].Amount);
            ...
            if (m_payments[i].C_Currency_ID_From.equals(as.m_C_Currency_ID)
              ...
              log4jDocInvoice.debug("Masuk if Currency ==");
              ...
              try {
                ...
                for (int j = 0; j < prepayments.length; j++) {
                  ...
                  log4jDocInvoice.debug("Masuk Looping <prePaymentAmt> Amount : " + prePaymentAmt);
          ...
          /*//added by GAI Team - untuk update cashflow, jika Fact_Acct di salah satu record memiliki Cashflow Y maka update semua menjadi Y
          String cashflowRec = FactLineData.selectCashflowYes(conn, AD_Table_ID, Record_ID);
          //String cashflowRec = FactLineData.selectIsCashflow(conn, Account_ID);
          log4jDocInvoice.debug("DocInvoice - Table ID : "+AD_Table_ID+" Record ID : "+Record_ID);
          log4jDocInvoice.debug("DocInvoice - cashflowRec DEBUG : "+cashflowRec);

          //if (cashflowRec.equals("Y")) {
              FactLineData.updateFACashflow(con, conn, AD_Table_ID, Record_ID);
          //}

          log4jDocInvoice.debug("DocInvoice - After Update Fact Acct Cashflow");*/
          //end added
      } //end if isCustomSales
      else {
        BigDecimal percentageFinalAccount = BigDecimal.ZERO;
        BigDecimal taxesAmountTotal = BigDecimal.ZERO;
        BigDecimal taxToTransAccount = BigDecimal.ZERO;
        BigDecimal taxToFinalAccount = BigDecimal.ZERO;

        String docTypeName = DocInvoiceData.selectDocTypeName(conn, Record_ID);
        ...
        for (int i = 0; m_taxes != null && i < m_taxes.length; i++) {
          ...
          percentageFinalAccount = CashVATUtil._100;
          taxesAmountTotal = new BigDecimal(
          ...
          if (IsReversal.equals("Y")) {
            ...
            taxToFinalAccount = taxesAmountTotal.subtract(taxToTransAccount);
        ...
        if (p_lines != null && p_lines.length > 0) {
          ...
          log4jDocInvoice.debug("Masuk if, p_lines not null");
          for (int i = 0; i < p_lines.length; i++) {
            log4jDocInvoice.debug("Masuk Loop if isCustom");
            
            String amount = p_lines[i].getAmount();
            BigDecimal amountprod = (new BigDecimal(amount)).add(taxToFinalAccount).add(taxToTransAccount);
            log4jDocInvoice.debug("Amount ditambah Tax : "+amountprod.toString()+"="+amount+"+"+taxToFinalAccount.toString()+"+"+taxToTransAccount.toString());
            log4jDocInvoice.debug("Masuk yg diharapkan");
            log4jDocInvoice.debug("Product getAccountProduct : "+getAccountProduct(((DocLine_Invoice) p_lines[i]).getProduct(), as, true, docTypeName, conn));
            fact.createLine(p_lines[i], getAccountProduct(((DocLine_Invoice) p_lines[i]).getProduct(), as, true, docTypeName, conn), this.C_Currency_ID, amountprod.toString(), "",
        ...
        /*//added by GAI Team - untuk update cashflow, jika Fact_Acct di salah satu record memiliki Cashflow Y maka update semua menjadi Y
        FactLineData[] cashflowRec = FactLineData.selectCashflowYes(conn, AD_Table_ID, Record_ID);
        //String cashflowRec = FactLineData.selectIsCashflow(conn, Account_ID);
        for (int i=0; i<=cashflowRec.length; i++){
          String isCS = cashflowRec[i].iscashflowyes;  
          log4jDocInvoice.debug("DocInvoice - cashflowRec DEBUG : "+isCS);
        }
      ...
      } //end else Receivable DR w/ m_payments != null
      ...
      // Charge CR
      log4jDocInvoice.debug("The first create line"); //AcctServer.ACCTTYPE_Charge
      fact.createLine(null, getAccount(AcctServer.ACCTTYPE_Charge, as, conn), C_Currency_ID, "",
          ...
          conn);
      log4jDocInvoice.debug("Product Receivable : "+ProductInfo.ACCTTYPE_P_Receivable);
      // TaxDue CR
      ...
      if (p_lines != null && p_lines.length > 0) {
        for (int i = 0; i < p_lines.length; i++) {
          ...
          Account account1 = ((DocLine_Invoice) p_lines[i]).getAccount(
              ProductInfo.ACCTTYPE_P_Receivable, as, conn);
          ...
          String amount = p_lines[i].getAmount();
          //BigDecimal amountbg = BigDecimal.ZERO; //add by GAI Team
          //BigDecimal amountbd = BigDecimal.ZERO; //add by GAI Team
          ...
          if (((DocLine_Invoice) p_lines[i]).isDeferred()) {
            ...
            if (IsReversal.equals("Y")) {
              ...
              //amountbg = new BigDecimal(amount);
              //amountbd = amountbg.divide(new BigDecimal(2));
              //log4jDocInvoice.debug("Ambil Revenue di bagian ini: " + p_lines[i].toString());
              /*fact.createLine(p_lines[i], account1, this.C_Currency_ID, amount, "",
                  Fact_Acct_Group_ID, nextSeqNo(SeqNo), DocumentType, conn);*/
              fact.createLine(p_lines[i], account, this.C_Currency_ID, "", amount,
                  Fact_Acct_Group_ID, nextSeqNo(SeqNo), DocumentType, conn);
            }
    ...
    // ARC
      ...
      for (int i = 0; p_lines != null && i < p_lines.length; i++) {
        ...
        Account account = ((DocLine_Invoice) p_lines[i]).getAccount(ProductInfo.ACCTTYPE_P_Revenue,
            as, conn); //ACCTTYPE_P_Revenue ACCTTYPE_P_Receivable
        if (((DocLine_Invoice) p_lines[i]).isDeferred()) {
          amount = createAccDefRevenueFact(fact, (DocLine_Invoice) p_lines[i], account,
      ...
      if (isPurchase.equals("N")) {
        if (m_payments == null || m_payments.length == 0)
          ...
          for (int i = 0; m_taxes != null && i < m_taxes.length; i++) {
            ...
            percentageFinalAccount = CashVATUtil._100;
            taxesAmountTotal = new BigDecimal(StringUtils.isBlank(m_taxes[i]
                .getAmount()) ? "0" : m_taxes[i].getAmount());
            taxToTransAccount = BigDecimal.ZERO;
            if (isCashVAT && m_taxes[i].m_isCashVAT) {
              percentageFinalAccount = CashVATUtil.calculatePrepaidPercentageForCashVATTax(
                  m_taxes[i].m_C_Tax_ID, Record_ID);
              taxToTransAccount = CashVATUtil.calculatePercentageAmount(
          ...
          if (p_lines != null && p_lines.length > 0) {
            log4jDocInvoice.debug("Masuk if, Purchase");
            for (int i = 0; i < p_lines.length; i++) {
              log4jDocInvoice.debug("Masuk Loop if isCustom");
              ...
              log4jDocInvoice.debug("Masuk yg diharapkan");
              log4jDocInvoice.debug("Product getAccountProduct : "+getAccountProduct(((DocLine_Invoice) p_lines[i]).getProduct(), as, true, docTypeName, conn));
              fact.createLine(p_lines[i], getAccountProduct(((DocLine_Invoice) p_lines[i]).getProduct(), as, false, docTypeName, conn), this.C_Currency_ID, "",amountprod.toString(),
                  Fact_Acct_Group_ID, nextSeqNo(SeqNo), DocumentType, conn);
            } //end for
          }
        }
   ...
   * @return Account
   */
  public final Account getAccountProduct(String mProductId, AcctSchema as, boolean isSoTrx,
      String docType,    
      ConnectionProvider conn) {
    ProductInfoData[] data = null;
    try {
      if (log4j.isDebugEnabled())
        log4j.debug("DocInvoice - getAccountProduct - DocumentType = " + DocumentType);
    ...
    // No account
    if (Account_ID.equals("")) {
      log4j.warn("DocInvoice - getAccountProduct - NO Product=" + mProductId
          + ", Record=" + Record_ID);
      return null;
    }
    // Return Account
