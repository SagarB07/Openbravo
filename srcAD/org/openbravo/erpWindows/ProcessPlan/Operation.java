...
vars.getGlobalVariable("inpmaProcessplanVersionId", windowId + "|MA_Processplan_Version_ID");
  ...
  private void refreshParentSession(VariablesSecureApp vars, String strPMA_Processplan_Version_ID) throws IOException,ServletException {
      ...
      if (data==null || data.length==0) return;
          vars.setSessionValue(windowId + "|MA_Processplan_Version_ID", data[0].maProcessplanVersionId);    vars.setSessionValue(windowId + "|AD_Org_ID", data[0].adOrgId);    vars.setSessionValue(windowId + "|AD_Client_ID", data[0].adClientId);
  ...
  private String getParentID(VariablesSecureApp vars, String strMA_Sequence_ID) throws ServletException {
    ...
    private void refreshSessionEdit(VariablesSecureApp vars, FieldProvider[] data) {
      if (data==null || data.length==0) return;
          vars.setSessionValue(windowId + "|AD_Client_ID", data[0].getField("adClientId"));    vars.setSessionValue(windowId + "|MA_Sequence_ID", data[0].getField("maSequenceId"));    vars.setSessionValue(windowId + "|AD_Org_ID", data[0].getField("adOrgId"));
    }
